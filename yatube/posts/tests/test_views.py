...
class PostViewTests(TestCase):
    ...
    def test_pages_uses_correct_template(self):
        ...
        templates_pages_names.update(self.create_new_post)
    ...
    def test_post_detail_pages_show_correct_context(self):
        self.client.post(reverse(
            'posts:add_comment', kwargs={'post_id': f'{self.post[0].pk}'}),
        data={
        'text': 'Тестовый комментарий2',
        'post_id': f'{self.post[0].pk}',},
        follow=True
        )
        response = self.client.get(reverse(
            'posts:post_detail',
            kwargs={'post_id': f'{self.post[0].pk}'}))
        ...
        """Проверяем контекс на содержимое комментариев"""
        self.assertEqual(response.context['comments'].first().text, 'Тестовый комментарий2')
